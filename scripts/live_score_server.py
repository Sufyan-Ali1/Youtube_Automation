...
Run:
    python scripts/live_score_server.py --fixture-id 123456
...
<head>
  ...
  <style>
    :root {
      ...
      --muted: #ced1d7;
      --accent: #ff2d2d;
      --accent-soft: rgba(255, 45, 45, .18);
      --gold: #f7d86a;
      --panel: rgba(7, 7, 10, .84);
      --panel-strong: rgba(5, 5, 8, .92);
      --line: rgba(255, 60, 60, .28);
      --shadow: 0 20px 90px rgba(0, 0, 0, .42);
    }
    * { box-sizing: border-box; }
    ...
    .stage {
      ...
      overflow: hidden;
      background:
        linear-gradient(180deg, rgba(0,0,0,0), rgba(0,0,0,.18) 38%, rgba(0,0,0,.68) 100%),
    ...
    .stage::before {
      ...
      inset: 0;
      background:
        radial-gradient(circle at 50% 12%, rgba(255,45,45,.10), transparent 24%),
        linear-gradient(180deg, rgba(0, 0, 0, 0), rgba(18,0,0,.06) 30%, rgba(0,0,0,.44) 68%, rgba(0,0,0,.78) 100%);
      pointer-events: none;
    }
    ...
    .live-pill {
      ...
      font-size: 28px;
      letter-spacing: .8px;
      box-shadow: 0 10px 26px rgba(255,45,45,.26);
    }
    .live-pill .dot {
      width: 14px;
      height: 14px;
    ...
    .minute-live.is-finished {
      ...
      border: 1px solid rgba(143,29,29,.55);
      border-radius: 999px;
      padding: 4px 10px;
    ...
    .status-chip {
      ...
      font-size: 34px;
      line-height: 1;
      box-shadow: inset 0 0 0 1px rgba(255,45,45,.18), 0 10px 24px rgba(0,0,0,.36);
    }
    .status-chip.is-finished {
      border-color: #a32020;
      background: rgba(22, 4, 4, .98);
      color: #ff7474;
      box-shadow: inset 0 0 0 1px rgba(163,32,32,.34), 0 10px 24px rgba(0,0,0,.5);
    }
    .card {
      margin-top: 16px;
      border-radius: 22px;
      border: 1px solid rgba(255,45,45,.88);
    ...
    .goal-card {
      ...
      box-shadow: inset 0 1px 0 rgba(255,45,45,.45), var(--shadow);
      padding-top: 8px;
    }
    .events-card {
      width: 960px;
      ...
      box-shadow: inset 0 1px 0 rgba(255,45,45,.45), inset 0 -1px 0 rgba(255,45,45,.45), var(--shadow);
    }
    .stats-card {
      width: 960px;
      min-height: 196px;
    ...
    .rank-box {
      width: 44px;
      height: 44px;
      border: 2px solid rgba(255,45,45,.88);
      border-radius: 8px;
      display: grid;
      place-items: center;
      color: var(--accent);
    ...
    .ticker {
      ...
      align-items: center;
      overflow: hidden;
      border-top: 1px solid rgba(255,45,45,.75);
      border-bottom: 1px solid rgba(255,45,45,.75);
      border-left: 1px solid rgba(255,45,45,.3);
      border-right: 1px solid rgba(255,45,45,.3);
      border-radius: 0;
      background: rgba(18, 6, 8, .92);
      box-shadow: 0 0 24px rgba(255,45,45,.22), inset 0 0 0 1px rgba(255,45,45,.22);
    }
    .ticker-track {
      white-space: nowrap;
      font-family: "Arial Narrow", "Roboto Condensed", "Trebuchet MS", sans-serif;
...
<body>
  ...
  <script>
    ...
    function renderStandings(standings, knockoutMessage) {
      ...
      knockoutWrap.textContent = knockoutMessage || 'Standings unavailable for this match.';
    }
    ...
    function render(data, stale = false) {
      ...
      renderStandings(data.standings, data.knockout_message || '');
